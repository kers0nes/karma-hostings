...
async function handleCommand(interaction) {
  ...
  if (commandName === 'obfuscate') {
    ...
    try {
      ...
      const obfuscated = await callObfuscator(code.slice(0, 4000), level);
  ...
  if (commandName === 'hostscript') {
    ...
    try {
      const finalCode = shouldObfuscate ? await callObfuscator(originalCode, level) : originalCode;
      const hosted = createHostedScript({
        guildId: interaction.guildId,
        name,
        code: String(finalCode),
...
\`\`\``
      ...
      await interaction.editReply({ content: `Hosting failed: ${error.message}` });
    }
    return;
  }
...
<head>
  ...
  <style>
    :root{--bg:#030303;--card:rgba(15,15,16,0.85);--muted:#a1a1aa;--line:rgba(212,175,55,0.2);--text:#f8fafc;--primary:#d4af37;--accent:#d4af37;--soft:#151518;--gold:#d4af37;--gold-glow:rgba(212,175,55,0.5)}
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:linear-gradient(rgba(0,0,0,0.7),rgba(0,0,0,0.7)),url("/assets/bg.png") center/cover fixed no-repeat,#030303;color:var(--text);font-family:"SF Pro Display","Aptos","Segoe UI Variable","Segoe UI",Inter,system-ui,sans-serif;letter-spacing:-.01em}body:before{content:'';position:fixed;right:-170px;bottom:-170px;width:620px;height:620px;background:url('/assets/karma-logo.png') center/contain no-repeat;opacity:.045;filter:grayscale(1);pointer-events:none}.grid{position:fixed;inset:0;background-image:linear-gradient(rgba(212,175,55,0.1) 1px,transparent 1px),linear-gradient(90deg,rgba(212,175,55,0.1) 1px,transparent 1px);background-size:64px 64px;mask-image:linear-gradient(to bottom,#000,transparent 82%);pointer-events:none}a{color:inherit;text-decoration:none}.container{width:min(1180px,92%);margin:auto}header{position:sticky;top:0;z-index:40;border-bottom:1px solid rgba(212,175,55,0.15);background:rgba(3,3,3,.82);backdrop-filter:blur(18px)}.nav{height:64px;display:flex;align-items:center;justify-content:space-between}.brand{position:absolute;left:50%;transform:translateX(-50%);display:flex;align-items:center;gap:10px;font-weight:780}.brand img{width:34px;height:34px;border-radius:10px;object-fit:cover;border:1px solid rgba(212,175,55,0.5)}.beta{font:10px ui-monospace,monospace;text-transform:uppercase;letter-spacing:.12em;border:1px solid #2d2d32;border-radius:5px;padding:2px 6px;color:#f1d592}.menu{width:38px;height:38px;display:grid;place-items:center;border:1px solid #2b2b30;border-radius:10px;background:rgba(212,175,55,0.05);color:var(--text)}.btn{display:inline-flex;align-items:center;gap:10px;border-radius:10px;border:1px solid rgba(212,175,55,0.4);background:rgba(212,175,55,0.1);padding:13px 18px;font:800 12px ui-monospace,monospace;text-transform:uppercase;letter-spacing:.12em;color:var(--text)}.btn.primary{background:var(--gold);color:#050505;border-color:var(--text);box-shadow:0 0 40px rgba(212,175,55,0.3)}.hero{position:relative;text-align:center;padding:105px 0 80px}.pill{display:inline-flex;gap:10px;align-items:center;border:1px solid rgba(212,175,55,0.4);background:rgba(212,175,55,0.08);border-radius:999px;padding:8px 13px;font:700 11px ui-monospace,monospace;text-transform:uppercase;letter-spacing:.16em;color:#f1d592}.pulse{width:7px;height:7px;border-radius:50%;background:var(--gold);box-shadow:0 0 18px var(--gold)}.hero h1{font-size:clamp(50px,8vw,104px);line-height:1.02;letter-spacing:-.075em;margin:26px auto 18px;max-width:930px}.glow{text-shadow:0 0 32px rgba(212,175,55,0.4)}.hero p{max-width:680px;margin:0 auto;color:#e2c98d;font:500 15px/1.8 ui-monospace,monospace}.actions{display:flex;justify-content:center;gap:12px;flex-wrap:wrap;margin-top:34px}.heroVideo{margin:56px auto 0;max-width:860px;border:1px solid rgba(212,175,55,0.2);border-radius:24px;overflow:hidden;background:linear-gradient(180deg,#111,#070707);box-shadow:0 0 80px rgba(212,175,55,0.2)}.fakeVideo{aspect-ratio:16/9;display:grid;place-items:center;background:radial-gradient(circle at 50% 40%,rgba(212,175,55,0.2),transparent 20%),linear-gradient(135deg,#050505,#151515,#050505);background-size:160% 160%;animation:shift 7s infinite alternate}.fakeVideo img{width:110px;height:110px;border-radius:28px;object-fit:cover;filter:grayscale(1);opacity:.9}@keyframes shift{to{background-position:100% 60%}}.caption{padding:14px;font:700 11px ui-monospace,monospace;text-transform:uppercase;letter-spacing:.16em;color:var(--gold)}.section{border-top:1px solid rgba(212,175,55,0.15);padding:88px 0}.sectionHead{max-width:720px;margin-bottom:34px}.kicker{font:800 12px ui-monospace,monospace;text-transform:uppercase;letter-spacing:.16em;color:var(--text);margin-bottom:10px}.section h2{font-size:clamp(34px,5vw,58px);line-height:1.02;letter-spacing:-.055em;margin:0}.muted{color:#e2c98d}.features{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}.card{border:1px solid rgba(212,175,55,0.2);border-radius:28px;background:rgba(15,15,16,.72);padding:26px;transition:.2s ease;box-shadow:inset 0 1px 0 rgba(212,175,55,0.05)}.card:hover{border-color:rgba(212,175,55,.35);transform:translateY(-2px);box-shadow:0 0 60px rgba(212,175,55,0.1)}.icon{width:38px;height:38px;display:grid;place-items:center;border:1px solid rgba(212,175,55,0.2);border-radius:12px;margin-bottom:16px}.card h3{margin:0 0 8px;font-size:18px}.card p{margin:0;color:#e2c98d;font:500 12px/1.7 ui-monospace,monospace}.stats{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.stat{border:1px solid rgba(212,175,55,0.2);border-radius:18px;background:rgba(15,15,16,.65);padding:22px;display:flex;gap:15px;align-items:center}.num{font-size:34px;font-weight:850}.pricing{display:grid;grid-template-columns:1fr 1fr;gap:0;border:1px solid rgba(212,175,55,0.2);border-radius:28px;overflow:hidden;background:rgba(15,15,16,.45)}.plan{padding:32px}.plan+ .plan{border-left:1px solid rgba(212,175,55,0.2);background:rgba(212,175,55,0.05)}.price{font-size:64px;font-weight:900;letter-spacing:-.06em}.plan ul{list-style:none;padding:0;margin:22px 0;display:grid;gap:13px}.plan li:before{content:'✓';margin-right:10px}.cta{text-align:center;max-width:760px;margin:auto}.footer{border-top:1px solid rgba(212,175,55,0.15);padding:34px 0;color:var(--gold);font:700 11px ui-monospace,monospace;text-transform:uppercase;letter-spacing:.16em;display:flex;justify-content:space-between;gap:16px;flex-wrap:wrap}@media(max-width:850px){.features,.stats,.pricing{grid-template-columns:1fr}.plan+.plan{border-left:0;border-top:1px solid rgba(212,175,55,0.2)}.brand{position:static;transform:none}.nav{gap:12px}.hero{text-align:left}.actions{justify-content:flex-start}}
  </style>
</head>
<body>
  <div class="grid"></div>
...
function discordDashboardPage(user, req = { query: {} }) {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Karma Dashboard</title><style>:root{--bg:#000000;--shell:rgba(11,11,12,0.9);--panel:rgba(16,16,17,0.8);--panel2:rgba(21,21,22,0.8);--line:rgba(212,175,55,0.25);--muted:#a1a1aa;--text:#f8fafc;--gold:#d4af37;--gold-glow:rgba(212,175,55,0.5);--gold2:#f1d592}*{box-sizing:border-box}body{margin:0;min-height:100vh;background:linear-gradient(rgba(0,0,0,0.75),rgba(0,0,0,0.75)),url("/assets/bg.png") center/cover fixed no-repeat,#000000;color:var(--text);font-family:"SF Pro Display","Aptos","Segoe UI Variable","Segoe UI",Inter,system-ui,sans-serif;letter-spacing:-.01em}body:before{content:'';position:fixed;right:-140px;bottom:-140px;width:560px;height:560px;background:url('/assets/karma-logo.png') center/contain no-repeat;opacity:.045;filter:grayscale(1);pointer-events:none}a{color:inherit;text-decoration:none}.page{padding:28px}.shell{max-width:1500px;margin:0 auto;min-height:calc(100vh - 56px);display:grid;grid-template-columns:280px 270px 1fr;border:1px solid rgba(212,175,55,0.4);border-radius:34px;overflow:hidden;background:linear-gradient(180deg,rgba(212,175,55,0.1),rgba(212,175,55,.015));box-shadow:0 34px 140px rgba(0,0,0,.58),0 0 0 1px rgba(212,175,55,.025)}.side{background:linear-gradient(180deg,rgba(12,12,12,.96),rgba(5,5,5,.96));border-right:1px solid rgba(212,175,55,0.3);padding:22px;overflow:auto;box-shadow:18px 0 80px rgba(0,0,0,.28)}.brand{display:flex;gap:12px;align-items:center;border-bottom:1px solid #242427;padding-bottom:20px}.brand img,.avatar{width:48px;height:48px;border-radius:16px;object-fit:cover;border:1px solid rgba(212,175,55,0.6);box-shadow:0 0 35px rgba(212,175,55,0.3)}.brand b{display:block;font-size:18px;font-weight:850}.brand small,.muted,small{color:var(--muted)}.nav{margin-top:20px}.nav a{display:flex;align-items:center;padding:12px 13px;border-radius:14px;color:#f1d592;font-weight:720;margin-bottom:4px}.nav a:hover,.nav a.active{background:linear-gradient(90deg,rgba(212,175,55,0.2),rgba(212,175,55,0.05));color:var(--text);box-shadow:inset 3px 0 0 var(--gold)}.scriptsPane{background:rgba(7,7,7,.78);border-right:1px solid rgba(212,175,55,0.2);padding:20px;overflow:auto}.scriptLink{display:block;border:1px solid rgba(212,175,55,0.25);background:rgba(14,14,14,.86);border-radius:16px;padding:13px;margin-bottom:10px}.scriptLink.active{border-color:var(--gold);background:linear-gradient(180deg,rgba(212,175,55,0.25),rgba(18,18,18,.88));box-shadow:0 10px 34px rgba(212,175,55,0.2)}.main{padding:28px;min-width:0;position:relative;overflow:hidden}.top{display:flex;justify-content:space-between;align-items:center;margin-bottom:20px}.profile{display:flex;gap:12px;align-items:center}.card{border:1px solid rgba(212,175,55,0.2);border-radius:28px;background:linear-gradient(180deg,rgba(24,24,24,.92),rgba(8,8,8,.97));padding:28px;box-shadow:inset 0 1px 0 rgba(212,175,55,.06),0 26px 90px rgba(0,0,0,.32);margin-bottom:18px;position:relative;z-index:1}.card h2{font-size:clamp(32px,4vw,56px);line-height:.95;letter-spacing:-.06em;margin:6px 0 12px}.eyebrow{color:#e2c98d;text-transform:uppercase;letter-spacing:.18em;font-size:12px;font-weight:850;margin:0 0 8px}.btn,button{display:inline-flex;align-items:center;justify-content:center;border:1px solid var(--gold2);background:linear-gradient(180deg,var(--gold2),var(--gold));color:#000;border-radius:999px;padding:12px 18px;font-weight:950;cursor:pointer;transition:transform .18s ease,box-shadow .18s ease,border-color .18s ease;box-shadow:0 12px 38px rgba(212,175,55,0.2)}.btn:hover,button:hover{transform:translateY(-1px);box-shadow:0 14px 42px rgba(212,175,55,0.15)}.btn.dark{background:rgba(10,10,10,.75);color:var(--text);border-color:rgba(212,175,55,0.4);box-shadow:none}.secondary{background:rgba(10,10,10,.75);color:var(--text);border-color:rgba(212,175,55,0.4)}.buttonRow{display:flex;gap:12px;flex-wrap:wrap;align-items:center;margin-top:6px}.danger{background:#220f0f;color:#ffb4ad;border-color:#5b2521}.stats{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-top:18px}.stat{border:1px solid rgba(212,175,55,0.25);border-radius:18px;background:rgba(10,10,10,.82);padding:18px}.num{font-size:38px;font-weight:900;letter-spacing:-.05em}select{background:rgba(8,8,9,0.9);color:var(--text);border:1px solid #343438;border-radius:14px;padding:10px;font:inherit}.inlineForm{display:flex;gap:10px;flex-wrap:wrap;margin-top:10px}input,textarea{width:100%;background:rgba(8,8,9,0.9);color:var(--text);border:1px solid #343438;border-radius:14px;padding:12px;margin:8px 0 14px;font:inherit}textarea{min-height:260px}.check{display:flex;gap:10px;align-items:center}.check input{width:auto}.block{display:block;white-space:pre-wrap;word-break:break-all;padding:12px;margin:10px 0;background:rgba(8,8,9,0.9);border:1px solid #343438;border-radius:14px}.row{border:1px solid #27272a;border-radius:14px;padding:12px;margin:8px 0;background:rgba(11,11,12,0.9)}.featureGrid,.stepsDash{display:grid;grid-template-columns:repeat(2,1fr);gap:14px;margin-top:16px}.featureGrid div,.stepsDash div{border:1px solid #27272a;border-radius:16px;background:rgba(11,11,12,0.9);padding:16px}.stepsDash span{display:inline-grid;place-items:center;width:32px;height:32px;border-radius:50%;background:var(--gold);color:#000;font-weight:900}.anime{height:220px;border-radius:24px;border:1px solid #27272a;margin-top:22px;background:radial-gradient(circle at 30% 50%,rgba(212,175,55,0.2),transparent 18%),radial-gradient(circle at 70% 50%,rgba(212,175,55,0.15),transparent 20%),linear-gradient(120deg,#000,#111,#000);background-size:160% 160%;animation:movebg 6s infinite alternate;position:relative;overflow:hidden}.anime:after{content:'';position:absolute;inset:-40%;background:conic-gradient(from 0deg,transparent,rgba(212,175,55,0.15),transparent 35%);animation:spin 8s linear infinite}@keyframes movebg{to{background-position:100% 60%}}@keyframes spin{to{transform:rotate(360deg)}}@media(max-width:1100px){.page{padding:12px}.shell{grid-template-columns:1fr;border-radius:22px}.side,.scriptsPane{border-right:0;border-bottom:1px solid var(--line)}.stats,.featureGrid,.stepsDash{grid-template-columns:1fr}.top{align-items:flex-start;gap:16px;flex-direction:column}}</style></head><body><div class="page"><div class="shell"><aside class="side"><div class="brand"><img src="/assets/karma-logo.png"><div><b>Karma Protection</b><small>${username}</small></div></div><nav class="nav"><a class="${tab==='overview'?'active':''}" href="/dashboard">Overview</a><a class="${tab==='scripts'?'active':''}" href="/dashboard?tab=scripts">Scripts</a><a class="${tab==='obfuscate'?'active':''}" href="/dashboard?tab=obfuscate">Obfuscate</a>${isOwner?`<a class="${tab==='owner'?'active':''}" href="/dashboard?tab=owner">Owner Panel</a>`:''}<a href="/logout">Logout</a></nav></aside><aside class="scriptsPane"><h3>Scripts</h3>${scriptLinks}<a class="btn" href="/dashboard?tab=scripts">New Script</a></aside><main class="main"><div class="top"><div class="profile"><img class="avatar" src="${avatar}"><div><b>${username}</b><br><small>${myScriptCount}/${scriptQuota} scripts used</small></div></div><div class="buttonRow"><a class="btn dark" href="/dashboard?tab=obfuscate">Obfuscator</a><a class="btn dark" href="/">Home</a></div></div>${content}</main></div></div><script>document.getElementById('fileInput')?.addEventListener('change', async e => { const f=e.target.files[0]; if(!f) return; document.querySelector('input[name="name"]').value ||= f.name.replace(/\.(lua|txt)$/i,''); document.getElementById('codeBox').value = await f.text(); });</script></body></html>`;
}
...
function startApiServer() {
  ...
  app.post('/dashboard/scripts/:id/update', async (req, res) => {
    ...
    // SYNC TO SUPABASE TO PREVENT REVERTS ON STARTUP
    const updated = db.prepare('SELECT * FROM hosted_scripts WHERE id = ?').get(req.params.id);
    if (updated) {
      saveHostedScriptToSupabase({
        ...updated,
        obfuscated: Boolean(updated.obfuscated)
      }).catch(err => console.warn('Supabase update sync failed:', err.message));
    }
  ...
  app.post('/dashboard/obfuscate', async (req, res) => {
    ...
    const level = String(req.body.level || 'standard');
    const saveToId = req.body.save_to_id; // Optional field to save result back to a script
    ...
    try {
      ...
      if (saveToId) {
        db.prepare('UPDATE hosted_scripts SET code = ?, source_code = ?, obfuscated = 1 WHERE id = ?')
          .run(obfuscated, code, saveToId);
        const updated = db.prepare('SELECT * FROM hosted_scripts WHERE id = ?').get(saveToId);
        if (updated) saveHostedScriptToSupabase({...updated, obfuscated: true}).catch(() => null);
        return res.redirect(`/dashboard?tab=scripts&script=${encodeURIComponent(saveToId)}`);
      }

      return res.type('html').send(`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Obfuscated - Karma Protection</title><style>:root{--gold:#d4af37;--gold-glow:rgba(212,175,55,0.5);--bg:#000}body{margin:0;background:linear-gradient(rgba(0,0,0,0.8),rgba(0,0,0,0.8)),url("/assets/bg.png") center/cover fixed no-repeat,#000;color:var(--text);font-family:SF Pro Display,Aptos,Segoe UI,system-ui,sans-serif}.wrap{width:min(1100px,94%);margin:32px auto}.card{border:1px solid rgba(212,175,55,0.3);border-radius:28px;background:rgba(15,15,16,0.9);padding:24px;box-shadow:0 0 40px rgba(212,175,55,0.1)}textarea{width:100%;min-height:62vh;background:rgba(5,5,5,0.9);color:var(--text);border:1px solid var(--gold);border-radius:16px;padding:14px;font:12px ui-monospace,monospace}button,a{display:inline-flex;margin:10px 8px 18px 0;padding:12px 18px;border-radius:999px;border:1px solid var(--gold);background:var(--gold);color:#000;text-decoration:none;font-weight:950;cursor:pointer}.dark{background:rgba(0,0,0,0.5);color:var(--text);border-color:rgba(212,175,55,0.4)}</style></head><body><div class="wrap"><div class="card"><h1 style="color:var(--gold)">Obfuscated Successfully</h1><p>Level: <b>${escapeHtml(level)}</b>. Copy it below — no download needed.</p><button onclick="navigator.clipboard.writeText(document.getElementById('out').value)">Copy Obfuscated Code</button><a class="dark" href="/dashboard?tab=obfuscate">Back to Obfuscator</a><a class="dark" href="/dashboard?tab=scripts">Scripts</a><textarea id="out" spellcheck="false">${escapeHtml(obfuscated)}</textarea></div></div></body></html>`);
    } catch (e) {
      return res.status(500).type('html').send(`<h1>Obfuscation Failed</h1><p>${escapeHtml(e.message)}</p><a href="/dashboard?tab=obfuscate">Back</a>`);
    }
  });
