...
return(function(...)
  ...
  local function ${nProbe}()
    ...
    -- Some Luau/executor environments restrict setfenv; do not fail normal users for that.
    if setfenv and ${nGetfenv} then local _env={} ${nPcall}(setfenv,function() return true end,_env) end
    local _mt={}
    local _lock={}
...
function discordDashboardPage(user, req = { query: {} }) {
  ...
  if (['keys','how','tutorials','redeem'].includes(tab)) tab = 'overview';
  ...
  const remaining = user.id === OWNER_ID ? 'Unlimited' : Math.max(0, Number(scriptQuota) - myScriptCount);
